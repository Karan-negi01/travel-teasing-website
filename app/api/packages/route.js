...
export async function GET(request) {
  ...
  const packageType  = searchParams.get('package_type'); // 'FIT' | 'GROUPS' | 'WEEKEND'
  const fitGroup     = searchParams.get('fit_group');    // 'true' → only is_fit_group packages
  ...
  if (fitGroup === 'true') query = query.eq('is_fit_group', true);
