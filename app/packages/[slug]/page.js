...
import { MapPin, Clock, Users, CheckCircle, XCircle, ChevronDown, ChevronUp, MessageCircle, Download, Navigation } from 'lucide-react';
...
export default function PackageDetailPage({ params }) {
  ...
  // Title: first word italic, rest bold
  ...
  return (
    <div className="bg-white min-h-screen" style={{ fontFamily: "'Poppins', sans-serif" }}>
      ...
      {/* ── TITLE + META (above images, like DeshVidesh) ── */}
      <div className="container-max pt-6 pb-4">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
          {/* Left: Title + Duration */}
          <div className="lg:col-span-2">
            <h1 className="text-[40px] md:text-[48px] font-semibold text-[#1a1a1a] leading-tight mb-2">
              <span style={{ fontFamily: "'Playfair Display', serif", fontStyle: 'italic', fontWeight: 400 }}>{firstWord} </span>
              <span>{restWords}</span>
            </h1>
            <p className="text-[14px] text-gray-500">
              Duration: <span className="font-medium text-gray-700">{pkg.duration_nights}N/{pkg.duration_days}D</span>
            </p>
          </div>
          {/* Right: Destination / Pickup meta */}
          <div className="space-y-2 text-[14px] text-gray-600 lg:pt-2">
            <div className="flex items-start gap-2">
              <MapPin size={15} className="text-gray-400 mt-0.5 flex-shrink-0" />
              <div>
                <span className="text-gray-400">Destination:</span>
                <span className="ml-2 font-medium text-gray-700">{pkg.location}{pkg.state && pkg.state !== pkg.location ? `, ${pkg.state}` : ''}</span>
              </div>
            </div>
            {pkg.departure_city && (
              <div className="flex items-start gap-2">
                <Navigation size={15} className="text-gray-400 mt-0.5 flex-shrink-0" />
                <div>
                  <span className="text-gray-400">Pickup/Drop:</span>
                  <span className="ml-2 font-medium text-gray-700">{pkg.departure_city}</span>
                </div>
              </div>
            )}
            {pkg.group_size_max > 0 && (
              <div className="flex items-start gap-2">
                <Users size={15} className="text-gray-400 mt-0.5 flex-shrink-0" />
                <div>
                  <span className="text-gray-400">Group Size:</span>
                  <span className="ml-2 font-medium text-gray-700">{pkg.group_size_min}–{pkg.group_size_max} people</span>
                </div>
              </div>
            )}
          </div>
      ...
      {/* ── IMAGE GALLERY (exact DeshVidesh: grid-cols-4, h-[30rem]) ── */}
      <div className="container-max pb-0">
        <div className="relative grid h-[30rem] grid-cols-1 gap-y-2 md:grid-cols-4 md:gap-3 2xl:h-[40rem] rounded-2xl overflow-hidden">
          {/* Main large image — col-span-2 */}
          <div className="group relative col-span-2 h-full cursor-pointer overflow-hidden">
            {allImages[0]
            ...
            <div className="absolute bottom-3 left-3 bg-white/80 backdrop-blur-sm rounded-full p-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35M11 8v6M8 11h6"/></svg>
            </div>
          </div>
          {/* Middle tall image — col-span-1 */}
          ...
          {/* Right: 2 stacked images — col-span-1 */}
          <div className="hidden h-full md:grid grid-cols-1 gap-3 col-span-1">
            <div className="group relative cursor-pointer overflow-hidden rounded-tr-2xl">
              {allImages[2]
            ...
            <div className="group relative cursor-pointer overflow-hidden rounded-br-2xl">
              {allImages[3]
              ...
              {allImages.length > 4 && (
                <div className="absolute inset-0 bg-black/40 flex items-center justify-center rounded-br-2xl">
                  <span className="text-white font-semibold text-sm">+{allImages.length - 4}</span>
                </div>
              )}
            </div>
          </div>
      ...
      {/* ── STICKY TABS ── */}
      <div className="sticky z-10 border-b border-gray-200 bg-white/90 backdrop-blur-xl" style={{ top: '72px' }}>
          ...
          <div className="flex gap-6 overflow-x-auto scrollbar-hide">
            {TABS.map(tab => (
              <button key={tab.id} onClick={() => scrollToSection(tab.id)}
                ...
                {tab.label}
                {activeTab === tab.id && <span className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#1a1a1a] rounded-full" />}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="container-max py-8 pb-24 lg:pb-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2">
            ...
            <div ref={el => sectionRefs.current['overview'] = el} className="scroll-mt-28 py-6">
                        ...
                        <div key={i}>
                          <button onClick={() => setOpenDay(isOpen ? null : i)}
                            ...
                            <div className="flex items-center gap-3 flex-1">
                              <span className="bg-gray-800 text-white text-[12px] font-semibold px-2.5 py-1 rounded-full flex-shrink-0">Day {day.day || i + 1}</span>
                              <h3 className="font-medium text-[#1a1a1a] text-[20px] leading-snug">{day.title}</h3>
                            </div>
            ...
            {/* Inclusions */}
            <div ref={el => sectionRefs.current['inclusions'] = el} className="scroll-mt-28 py-6 border-t border-gray-100">
            ...
            {/* Costing */}
            <div ref={el => sectionRefs.current['costing'] = el} className="scroll-mt-28 py-6 border-t border-gray-100">
              ...
              <div className="border border-gray-200 rounded-2xl overflow-hidden">
                ...
                {savings && (
                  <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between bg-green-50">
                    <span className="text-[15px] text-green-700">You Save</span>
                ...
                {pkg.tour_options?.map((opt, i) => (
                  <div key={i} className="px-5 py-4 border-b border-gray-100 last:border-b-0 flex items-center justify-between">
                    ...
                    <span className="text-[15px] font-semibold">₹{parseInt(opt.price).toLocaleString('en-IN')}</span>
                  </div>
                ))}
                <div className="px-5 py-3 bg-gray-50">
                  <p className="text-[12px] text-gray-400">* All prices are per person and exclude applicable taxes.</p>
                </div>
              </div>
            </div>

            {/* About Trip */}
            <div ref={el => sectionRefs.current['about'] = el} className="scroll-mt-28 py-6 border-t border-gray-100">
              <h2 className="text-[24px] font-bold text-[#1a1a1a] mb-5">About the Trip</h2>
              <p className="text-[16px] text-black leading-relaxed">{pkg.description}</p>
                  ...
                  <ul className="space-y-2">
                    {pkg.highlights.map((h, i) => (
                      <li key={i} className="flex items-start gap-2 text-[15px] text-gray-700">
                        <CheckCircle size={14} className="text-[#E8651A] mt-0.5 flex-shrink-0" />{h}
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          {/* RIGHT SIDEBAR */}
          <div>
            <div className="sticky rounded-lg border border-gray-800 bg-white shadow-sm" style={{ top: '130px' }}>
              <div className="p-5 border-b border-gray-200">
                <p className="text-[12px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Book This Tour</p>
                <div className="flex items-baseline gap-2 flex-wrap">
                  {displayPrice
                    ? <span className="text-[28px] font-black text-[#1a1a1a]">₹{displayPrice?.toLocaleString('en-IN')}</span>
                    : <span className="text-[18px] font-semibold text-gray-600">Price on Request</span>
                  }
                  {savings && <span className="text-gray-400 text-[13px] line-through">₹{pkg.original_price?.toLocaleString('en-IN')}</span>}
                  {savings && <span className="bg-green-100 text-green-700 text-[11px] font-semibold px-2 py-0.5 rounded-full">Save ₹{savings.toLocaleString('en-IN')}</span>}
                </div>
                {displayPrice && <p className="text-gray-400 text-[11px] mt-0.5">+ tax as applicable</p>}
              </div>

              <div className="p-5 space-y-4">
                {dateOptions.length > 0 && (
                  <div>
                    <label className="block text-[12px] font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Select Date</label>
                    <select value={JSON.stringify(selectedDate)} onChange={e => setSelectedDate(JSON.parse(e.target.value))}
                      className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-[14px] focus:outline-none focus:ring-2 focus:ring-[#1a1a1a] bg-white">
                      {dateOptions.map((d, i) => (
                ...
                {pkg.tour_options?.length > 0 && (
                  <div>
                    <p className="text-[12px] font-semibold text-gray-500 uppercase tracking-wider mb-2">Tour Option</p>
                    <div className="space-y-2">
                      {pkg.tour_options.map((opt, i) => (
                        <button key={i} type="button" onClick={() => setSelectedOption(opt)}
                          className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border-2 text-[14px] font-medium transition-all ${
                            selectedOption?.label === opt.label ? 'border-[#E8651A] bg-orange-50 text-[#E8651A]' : 'border-gray-200 text-gray-700 hover:border-gray-300'
                          }`}>
                          <span>{opt.label}</span>
                ...
                <button onClick={() => setEnquiryOpen(true)}
                  className="w-full bg-[#1a1a1a] hover:bg-[#333] text-white py-3.5 rounded-full font-bold text-[15px] transition-colors">
                  Book Now
                </button>
                <div className="flex gap-2">
                  <a href={pkg.itinerary_pdf || '#'}
                    className={`flex-1 flex items-center justify-center gap-2 border-2 border-gray-200 text-gray-700 py-2.5 rounded-full text-[13px] font-semibold hover:border-gray-300 transition-colors ${!pkg.itinerary_pdf ? 'opacity-50 pointer-events-none' : ''}`}>
                    <Download size={13} /> Download Itinerary
                  </a>
                  <a href={`https://wa.me/916396464369?text=${encodeURIComponent(waText)}`}
                    target="_blank" rel="noopener noreferrer"
                    className="w-11 h-11 flex items-center justify-center bg-[#25D366] text-white rounded-full hover:bg-green-500 transition-colors flex-shrink-0">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 px-4 py-3 flex items-center gap-3 shadow-xl">
        ...
        <a href={`https://wa.me/916396464369?text=${encodeURIComponent(waText)}`} target="_blank" rel="noopener noreferrer"
          className="flex items-center gap-2 bg-[#25D366] text-white px-4 py-2.5 rounded-full text-[13px] font-semibold">
          <MessageCircle size={14} /> WhatsApp
        </a>
        <button onClick={() => setEnquiryOpen(true)}
          className="flex items-center gap-2 bg-[#1a1a1a] text-white px-4 py-2.5 rounded-full text-[13px] font-semibold">
          Book Now
        </button>
      </div>
